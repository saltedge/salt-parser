  ...
  module Parser
    class BaseParser
      ...
      def check_for_errors
        ...
        statuses.each do |status|
          if status.search("severity").inner_text == "ERROR"
            errors << OFX::RequestError.new(["[#{status.search("code").inner_text}]", status.search("message").inner_text].join(" ").strip)
          end
        end
      end
      ...
      def parse_float(incoming, options={})
        ...
        raise OFX::ParseError.new(OFX::ParseError::FLOAT)
      end
